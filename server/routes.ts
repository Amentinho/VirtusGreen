...
import { listPhenologyProfiles } from "./verification/phenology";
...
function evaluatePixel(s) {
  ...
  // ── Certifier API ─────────────────────────────────────────────────────────
  // Read-only endpoints for GI inspection bodies, consortia, and importers.
  // Authentication: Bearer token via CERTIFIER_API_KEY env var.
  // All responses include a disclaimer that this is automated evidence, not legal certification.

  const certifierAuth = (req: any, res: any, next: any) => {
    const key = process.env.CERTIFIER_API_KEY;
    if (!key) { res.status(503).json({ error: "Certifier API not configured on this server" }); return; }
    const header = req.headers["authorization"] ?? "";
    const provided = header.startsWith("Bearer ") ? header.slice(7) : "";
    if (provided !== key) { res.status(401).json({ error: "Invalid or missing API key" }); return; }
    next();
  };

  const CERTIFIER_DISCLAIMER =
    "This record is an automated risk-screening report generated by VirtusGreen using " +
    "Copernicus Sentinel-2 satellite data and blockchain anchoring. It provides supporting " +
    "evidence for GI compliance under EU Regulation 2024/1143 but does not constitute legal " +
    "certification. Final GI certification remains the responsibility of accredited " +
    "conformity assessment bodies (CSQA, DNV, Bureau Veritas, etc.).";

  // GET /api/certifier/batch/:batchCode
  // Full verification record for one batch — primary endpoint for inspection bodies
  app.get("/api/certifier/batch/:batchCode", certifierAuth, async (req, res) => {
    try {
      if (!db) { res.status(503).json({ error: "Database not configured" }); return; }
      const [batch] = await db.select().from(batches).where(eq(batches.batchCode, req.params.batchCode));
      if (!batch) { res.status(404).json({ error: "Batch not found" }); return; }
      ...
      const [plot] = await db.select().from(plots).where(eq(plots.id, batch.plotId));
      res.json({
        _schema: "VirtusGreen Certifier API v1",
        _disclaimer: CERTIFIER_DISCLAIMER,
        batch: {
          ...
          ndviAvg: batch.ndviAvg,
          ndviConfidence: batch.ndviConfidence,
          verifiedAt: batch.verifiedAt,
          dppIssued: batch.dppIssued,
          dppIssuedAt: batch.dppIssuedAt,
          verificationDetails: batch.verificationDetails,
        },
        blockchain: {
          txHash: batch.txHash,
          chainId: batch.chainId,
          chainName: batch.chainId === 8453 ? "Base mainnet" : batch.chainId === 84532 ? "Base Sepolia testnet" : null,
          anchoredAt: batch.anchoredAt,
          explorerUrl: batch.txHash ? `https://basescan.org/tx/${batch.txHash}` : null,
        },
        producer: {
          name: producer?.name,
          farmName: producer?.farmName,
          ...
          region: producer?.region,
          giType: producer?.giType,
          giCertificationNumber: producer?.giCertificationNumber,
          certificationBody: producer?.certificationBody,
        },
        plot: {
          name: plot?.name,
          cadastralRef: plot?.cadastralRef,
          altitudeM: plot?.altitudeM,
  ...
  // GET /api/certifier/producer/:producerId/batches
  // All batches for a producer — useful for annual GI inspection reports
  app.get("/api/certifier/producer/:producerId/batches", certifierAuth, async (req, res) => {
    try {
      if (!db) { res.status(503).json({ error: "Database not configured" }); return; }
      const [producer] = await db.select().from(producers).where(eq(producers.id, req.params.producerId));
      if (!producer) { res.status(404).json({ error: "Producer not found" }); return; }
      const producerBatches = await db
        .select()
        .from(batches)
        .where(eq(batches.producerId, req.params.producerId))
        .orderBy(desc(batches.createdAt));
      const summary = {
        total: producerBatches.length,
        verified: producerBatches.filter(b => b.verificationStatus === "verified").length,
        rejected: producerBatches.filter(b => b.verificationStatus === "rejected").length,
        pending: producerBatches.filter(b => b.verificationStatus === "pending").length,
        anchored: producerBatches.filter(b => b.txHash).length,
      };
      res.json({
        _schema: "VirtusGreen Certifier API v1",
        _disclaimer: CERTIFIER_DISCLAIMER,
        ...
        summary,
        batches: producerBatches.map(b => ({
          batchCode: b.batchCode,
          skin: b.skin,
          harvestDateFrom: b.harvestDateFrom,
          harvestDateTo: b.harvestDateTo,
          quantityKg: b.quantityKg,
          verificationStatus: b.verificationStatus,
          ndviAvg: b.ndviAvg,
          ndviConfidence: b.ndviConfidence,
          verifiedAt: b.verifiedAt,
          txHash: b.txHash,
          anchoredAt: b.anchoredAt,
          dppIssued: b.dppIssued,
        })),
        generatedAt: new Date().toISOString(),
      });
    } catch (e: any) {
  ...
  // GET /api/certifier/batch/:batchCode/dds
  // EUDR Due Diligence Statement — Art. 9 Reg 2023/1115 compatible export
  // Returns structured JSON ready for filing with the EU TRACES portal
  app.get("/api/certifier/batch/:batchCode/dds", certifierAuth, async (req, res) => {
    try {
      if (!db) { res.status(503).json({ error: "Database not configured" }); return; }
      const [batch] = await db.select().from(batches).where(eq(batches.batchCode, req.params.batchCode));
      if (!batch) { res.status(404).json({ error: "Batch not found" }); return; }
      if (batch.verificationStatus !== "verified") {
        res.status(422).json({ error: "DDS can only be generated for verified batches", status: batch.verificationStatus });
        return;
      }
      const [producer] = await db.select().from(producers).where(eq(producers.id, batch.producerId));
      const [plot] = await db.select().from(plots).where(eq(plots.id, batch.plotId));
      const details = batch.verificationDetails as any;

      // Parse EUDR payload stored in varietyNotes for Modica IGP batches
      let eudrPayload: any = null;
      if (batch.skin === "modica" && batch.varietyNotes) {
        try { eudrPayload = JSON.parse(batch.varietyNotes); } catch {}
      }

      // Derive commodity from skin
      const COMMODITY_MAP: Record<string, { hsCode: string; commodity: string; scientificName: string }> = {
        bronte:  { hsCode: "0802.50", commodity: "Pistachios", scientificName: "Pistacia vera" },
        etna:    { hsCode: "2204",    commodity: "Wine / Grapes", scientificName: "Vitis vinifera" },
        modica:  { hsCode: "1801",    commodity: "Cocoa beans", scientificName: "Theobroma cacao" },
        yubari:  { hsCode: "0807.19", commodity: "Melons", scientificName: "Cucumis melo" },
      };
      const commodity = COMMODITY_MAP[batch.skin] ?? { hsCode: "N/A", commodity: batch.skin, scientificName: "N/A" };

      // Extract geolocation from plot polygon centroid
      const coords = (plot?.polygon as any)?.coordinates?.[0] as [number, number][] | undefined;
      const centroid = coords?.length
        ? {
            longitude: parseFloat((coords.reduce((s, c) => s + c[0], 0) / coords.length).toFixed(6)),
            latitude: parseFloat((coords.reduce((s, c) => s + c[1], 0) / coords.length).toFixed(6)),
          }
        : null;

      const dds = {
        _schema: "VirtusGreen EUDR Due Diligence Statement v1",
        _regulation: "EU Regulation 2023/1115 on deforestation-free products (EUDR)",
        _article: "Article 9 — Due Diligence Statement",
        _status: "AUTOMATED_EVIDENCE — This DDS contains machine-generated supporting evidence. " +
          "A human operator must review and formally submit the completed DDS to the EU TRACES NT portal. " +
          "Full FAO-compliant forest baseline verification and formal DDS submission are on the VirtusGreen roadmap for Q4 2026.",
        _generatedAt: new Date().toISOString(),
        _referenceNumber: `VG-DDS-${batch.batchCode}`,

        operator: {
          name: producer?.name,
          farmName: producer?.farmName,
          country: producer?.country ?? "IT",
          region: producer?.region,
        ...
        commodity: {
          ...
          quantityKg: batch.quantityKg ? parseFloat(String(batch.quantityKg)) : null,
          productionPeriod: {
            from: batch.harvestDateFrom,
            to: batch.harvestDateTo,
          },
          countryOfProduction: batch.skin === "modica"
            ? (eudrPayload?.originCountry ?? "Unknown — see EUDR supply chain data")
            : "IT",
        },
        ...
        deforestationEvidence: {
          method: "Copernicus Sentinel-2 NDVI satellite analysis",
          ndviAvg: batch.ndviAvg ? parseFloat(String(batch.ndviAvg)) : null,
          ndviConfidence: batch.ndviConfidence,
          verificationDate: batch.verifiedAt,
          layers: details?.layers ?? null,
          phenologyMatch: details?.phenology?.cropMatch ?? null,
          zoneCheck: details?.zone?.insideZone ?? null,
          baselineAnomaly: details?.baseline ?? null,
          satelliteDataSource: "EU Copernicus Sentinel-2 L2A via Sentinel Hub Statistics API",
          imageryPeriod: {
            from: batch.harvestDateFrom,
            to: batch.harvestDateTo,
          },
          // FAO forest baseline: roadmap Q4 2026
          faoForestBaseline: {
            status: "PENDING",
            note: "FAO-compliant forest/no-forest baseline check and deforestation risk score are on the VirtusGreen roadmap for Q4 2026, ahead of the December 2026 EUDR SME deadline.",
          },
        },

        // Modica-specific EUDR supply chain (cocoa importers)
        supplyChain: eudrPayload ? {
          supplierName: eudrPayload.supplierName,
          supplierCountry: eudrPayload.supplierCountry,
          originCountry: eudrPayload.originCountry,
          importDocRef: eudrPayload.importDocRef,
          certifications: eudrPayload.certifications,
          farmCoordinates: eudrPayload.farmCoordinates,
          processingTemperatureC: eudrPayload.processingTemp,
          eudrStatementSigned: eudrPayload.eudrStatement,
        } : null,

        blockchain: {
          txHash: batch.txHash,
          chainId: batch.chainId,
          chainName: batch.chainId === 8453 ? "Base mainnet (Ethereum L2)" : null,
          anchoredAt: batch.anchoredAt,
          explorerUrl: batch.txHash ? `https://basescan.org/tx/${batch.txHash}` : null,
          note: "The verification hash is anchored on Base blockchain for tamper-evident timestamping.",
        },

        operatorDeclaration: {
          text: "The operator declares that, to the best of their knowledge, the information above is accurate and that the product was produced in compliance with the applicable legislation of the country of production.",
          signedBy: null,   // must be completed by operator before formal submission
          signedAt: null,   // must be completed by operator before formal submission
          submittedToTraces: false,
        },
      };

      // Optionally return as file download
      const asFile = req.query.format === "file";
      if (asFile) {
        res.setHeader("Content-Type", "application/json");
        res.setHeader("Content-Disposition", `attachment; filename="DDS-${batch.batchCode}.json"`);
      }
      res.json(dds);
    } catch (e: any) {
      res.status(500).json({ error: e.message });
  ...
  // GET /api/certifier/phenology-profiles — list all registered crop phenology profiles
  app.get("/api/certifier/phenology-profiles", certifierAuth, (_req, res) => {
    res.json({
      _schema: "VirtusGreen Certifier API v1",
      profiles: listPhenologyProfiles(),
      note: "Each profile defines the expected monthly NDVI range for a specific GI crop. New zones can be registered without code changes via registerPhenologyProfile().",
    });
  });

  // GET /api/certifier/ping — health check for API key validation
  app.get("/api/certifier/ping", certifierAuth, (_req, res) => {
    res.json({
      status: "ok",
      api: "VirtusGreen Certifier API v1",
      endpoints: [
        "GET /api/certifier/batch/:batchCode — full verification record",
        "GET /api/certifier/batch/:batchCode/dds — EUDR Due Diligence Statement (JSON)",
        "GET /api/certifier/batch/:batchCode/dds?format=file — DDS as downloadable JSON file",
        "GET /api/certifier/producer/:producerId/batches — all batches for a producer",
        "GET /api/certifier/phenology-profiles — list all registered crop profiles",
        "GET /api/certifier/ping — this endpoint",
      ],
      regulations: ["EU GI Reg 2024/1143", "EUDR 2023/1115", "Wine Reg 2021/2117"],
